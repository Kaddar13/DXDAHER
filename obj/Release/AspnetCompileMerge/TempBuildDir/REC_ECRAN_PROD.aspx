...
<%--        /*ligne des radio button*/--%>
    ...
    <asp:SqlDataSource ID="DS_Realise1" runat="server" ConnectionString="<%$ ConnectionStrings:laboConnectionString %>" SelectCommand="SELECT        COUNT(*) AS ObjectifGlobal
...
GROUP BY dbo.T_MACS_Suivi.Saisie_le"></asp:SqlDataSource>
    <asp:SqlDataSource ID="DS_Objectif1" runat="server" ConnectionString="<%$ ConnectionStrings:laboConnectionString %>" SelectCommand="IF DATEPART(HOUR, GETDATE()) &gt; 14 
BEGIN
...
SELECT [NB_LIGNE_OBJECTIF]FROM [T_MACS_OBJECTIF] WHERE ([NB_PERSONNE] = @Param1) AND (HEURE_OBJECTIF =14-DATEPART(HOUR, GETDATE()));">
    ...
    <asp:SqlDataSource ID="DS_Realise2" runat="server" ConnectionString="<%$ ConnectionStrings:laboConnectionString %>" SelectCommand="SELECT        COUNT(*) AS ObjectifGlobal
FROM            dbo.T_MACS_Suivi INNER JOIN
                         dbo.T_MACS_MATRICULE ON dbo.T_MACS_Suivi.Utilisateur = dbo.T_MACS_MATRICULE.XA_MATRICULE INNER JOIN
                         dbo.T_MACS_MVTD ON dbo.T_MACS_Suivi.Colonne15 = dbo.T_MACS_MVTD.MVTD
...
GROUP BY dbo.T_MACS_Suivi.Saisie_le"></asp:SqlDataSource>
    <asp:SqlDataSource ID="DS_Objectif2" runat="server" ConnectionString="<%$ ConnectionStrings:laboConnectionString %>" SelectCommand="IF DATEPART(HOUR, GETDATE()) &gt; 14 
BEGIN
